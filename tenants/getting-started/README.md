...
ConfigurationManager.mjs

```js
// core/ConfigurationManager.mjs
export class ConfigurationManager {
  static #instance = null;

  static getInstance(overrides = {}) {
    if (!ConfigurationManager.#instance) {
      ConfigurationManager.#instance = new ConfigurationManager(overrides);
    }
  ...
  constructor(overrides = {}) {
    if (ConfigurationManager.#instance) {
      throw new Error("Use ConfigurationManager.getInstance()");
    }
  ...
  #getDefaultConfig() {
    return {
      ...
      tenants: { maxConcurrent: 50, autoLoad: true },
    };
  }
...
# ITenantRepository.mjs

```js
// interfaces/ITenantRepository.mjs
...
// interfaces/IConfigurationProvider.mjs
  ...
  resolve(name) {
    ...
    if (dependency.singleton) {
      if (!this.singletons.has(name)) {
        const deps = dependency.dependencies.map((dep) => this.resolve(dep));
        this.singletons.set(name, dependency.factory(...deps));
      }
      return this.singletons.get(name);
    }

    const deps = dependency.dependencies.map((dep) => this.resolve(dep));
    return dependency.factory(...deps);
  }
}
```

# fastify-multitenant-enterprise/index.mjs
...
export default async function enterpriseTenant(fastify, options) {
  const { tenant, config, prefix } = options;

  // Register tenant-specific middleware
  fastify.addHook("onRequest", async (request, reply) => {
    request.log.info(`Enterprise tenant request: ${request.url}`);
  });

  // Register routes with tenant prefix
  fastify.register(async function (fastify) {
    fastify.get("/enterprise-dashboard", async (request, reply) => {
      const auditService = request.tenant.getService("auditService");
      return {
        tenant: "enterprise",
        auditLogs: await auditService.getRecentLogs(),
        userCount: await auditService.getUserCount(),
      };
    });

    fastify.post("/enterprise-users", async (request, reply) => {
      const userService = request.tenant.getService("enterpriseUserService");
      return await userService.createEnterpriseUser(request.body);
    });
  });
}
```

# fastify-multitenant-enterprise/package.json

```js
// fastify-multitenant-enterprise/package.json
{
  "name": "fastify-multitenant-enterprise",
  "version": "1.0.0",
  "type": "module",
  "main": "index.mjs",
  "keywords": ["fastify", "multitenant", "enterprise"],
  "dependencies": {
    "fastify": "^4.0.0"
  }
}
...
```md
fastify-multitenant-enterprise/
├── package.json
├── index.mjs # Main tenant export
├── config.json # Default configuration
├── routes/
│ └── index.mjs # Enterprise routes
├── services/
│ ├── enterpriseUserService.mjs
│ └── auditService.mjs
├── plugins/
│ └── sso/
│ └── index.mjs # SSO plugin
└── schemas/
├── audit.json
